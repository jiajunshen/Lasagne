...
def MultiGaussianMixture(X, targets):
    ...
    Theano 1D tensor
    """
    def __init__(self, incoming, num_components, n_classes, _means = init.GlorotUniform(), _sigma = 1, weights = None, sigma = None, **kwargs):
    ...
    def get_output_for(self,input, **kwargs):
        ...
        sigma_determinat = sum(self.sigma)
        exponentialResult = -T.sqr(T.dot(inputData - mean_reshape, sigma_inverse)) - T.log(sigma_determinat)
        logComponentSum = exponentialResult.sum(axis = 3) + T.log(weights_reshape)
